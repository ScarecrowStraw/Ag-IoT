...
//  int a2_read = 500;
  ...
  Serial.println(a1_read);
  Serial.println(a2_read);
  ...
  String data = "";
  data = data + "C0:" + String((a0_read * (max_volts / max_analog_steps))*1000);
  data = data + "NH3:" + String((a1_read * (max_volts / max_analog_steps))*500);
  data = data + "NO2:" + String((a2_read * (max_volts / max_analog_steps))*9.5);
  data = data + "Vis:" + String(uv.readVisible());
  data = data + "IR:" + String(uv.readIR());
  data = data + "UV:" + String(UVindex);

  String temp = "{";
  temp += "\"CO\":";
  temp += String((a0_read * (max_volts / max_analog_steps))*1000);
  temp += ",";
  temp += "\"NH3\":";
  temp += String((a1_read * (max_volts / max_analog_steps))*500);
  temp += ",";
  temp += "\"NO2\":";
  temp += String((a2_read * (max_volts / max_analog_steps))*9.5);
  temp += ",";
  temp += "\"Vis\":";
  temp += String(uv.readVisible());
  temp += ",";
  temp += "\"IR\":";
  temp += String(uv.readIR());
  temp += ",";
  temp += "\"UV\":";
  temp += String(UVindex);
  temp += "}";
